		...
		if (level_1 == true)
		{
				...
				for (int i = 0; i < max_rocks; i++)
				{
					if (gameover == false)
					{
						...
						if (rocks[i].spriteComponent()->getBoundingBox().isInside(cursor.spriteComponent()->getBoundingBox()) && rocks[i].visibility == true && leftMouseDown == true && fire == false && number_selected < 1)
						{
...
void AngryBirdsGame::render(const ASGE::GameTime &)
{
	...
	else if (!in_menu)
	{
		...
		if (gameover == true)
		{
			...
			//set pos
			gameover_sign_sprite->xPos(game_width / 2 - 200);
			gameover_sign_sprite->yPos(100);
			okay_sprite->xPos(game_width / 2 - 120);
			okay_sprite->yPos(480);
